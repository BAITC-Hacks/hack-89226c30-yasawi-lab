...
from agentic_forecast.agents import RunStore, run_first_origin
...
def test_api_contract(tmp_path: Path, monkeypatch):
    ...
    assert context.json()["capabilities"]["replay"] is False
    assert client.get("/api/runs/" + "0" * 32).status_code == 404
    assert client.post("/api/runs", json={"mode": "replay"}).status_code == 422
